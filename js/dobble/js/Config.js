define(["jquery", "knockout", "dobble/js/list/Marvel", "dobble/js/list/Legumes", "dobble/js/list/Fruits"], function($, ko, marvel, legumes, fruits) {
    return function Config() {
        var self = this;
        self.lists = {
            legumes: legumes,
            fruits: fruits,
            marvel: marvel
        };

        /***************
         * Observables *
         ***************/
        self.availableNbCards = ko.observableArray([7, 13, 21, 31, 57]);
        self.nbCards          = ko.observable(21);
        self.words            = ko.observableArray([]);
        self.list             = ko.observable("fruits");

        /*******************
         * Non-Observables *
         *******************/
        self.matrices = {
            "nb-cards-7": [
                [0, 1, 2],
                [0, 3, 4],
                [0, 5, 6],
                [1, 3, 5],
                [1, 4, 6],
                [2, 3, 6],
                [2, 4, 5]
            ],
            "nb-cards-13": [
                [0, 1, 2, 3],
                [0, 4, 5, 6],
                [0, 7, 8, 9],
                [0, 10, 11, 12],
                [1, 4, 7, 10],
                [1, 5, 8, 11],
                [1, 6, 9, 12],
                [2, 4, 8, 12],
                [2, 5, 9, 10],
                [2, 6, 7, 11],
                [3, 4, 9, 11],
                [3, 5, 7, 12],
                [3, 6, 8, 10]
            ],
            "nb-cards-21": [
                [2, 6, 10, 14, 18],
                [1, 6, 7, 8, 9],
                [3, 6, 11, 16, 21],
                [5, 6, 13, 15, 20],
                [4, 6, 12, 17, 19],
                [1, 2, 3, 4, 5],
                [2, 7, 11, 15, 19],
                [2, 9, 13, 17, 21],
                [2, 8, 12, 16, 20],
                [1, 10, 11, 12, 13],
                [3, 7, 10, 17, 20],
                [5, 9, 10, 16, 19],
                [4, 8, 10, 15, 21],
                [1, 18, 19, 20, 21],
                [4, 7, 13, 16, 18],
                [3, 9, 12, 15, 18],
                [5, 8, 11, 17, 18],
                [1, 14, 15, 16, 17],
                [5, 7, 12, 14, 21],
                [4, 9, 11, 14, 20],
                [3, 8, 13, 14, 19]
            ],
            "nb-cards-31": [
                [0, 1, 2, 3, 4, 5],
                [0, 6, 7, 8, 9, 10],
                [0, 11, 12, 13, 14, 15],
                [0, 16, 17, 18, 19, 20],
                [0, 21, 22, 23, 24, 25],
                [0, 26, 27, 28, 29, 30],
                [1, 6, 11, 16, 21, 26],
                [1, 7, 12, 17, 22, 27],
                [1, 8, 13, 18, 23, 28],
                [1, 9, 14, 19, 24, 29],
                [1, 10, 15, 20, 25, 30],
                [2, 6, 12, 18, 24, 30],
                [2, 7, 13, 19, 25, 26],
                [2, 8, 14, 20, 21, 27],
                [2, 9, 15, 16, 22, 28],
                [2, 10, 11, 17, 23, 29],
                [3, 6, 13, 20, 22, 29],
                [3, 7, 14, 16, 23, 30],
                [3, 8, 15, 17, 24, 26],
                [3, 9, 11, 18, 25, 27],
                [3, 10, 12, 19, 21, 28],
                [4, 6, 14, 17, 25, 28],
                [4, 7, 15, 18, 21, 29],
                [4, 8, 11, 19, 22, 30],
                [4, 9, 12, 20, 23, 26],
                [4, 10, 13, 16, 24, 27],
                [5, 6, 15, 19, 23, 27],
                [5, 7, 11, 20, 24, 28],
                [5, 8, 12, 16, 25, 29],
                [5, 9, 13, 17, 21, 30],
                [5, 10, 14, 18, 22, 26]
            ],
            "nb-cards-57": [
                [0, 1, 2, 3, 4, 5, 6, 7],
                [0, 8, 9, 10, 11, 12, 13, 14],
                [0, 15, 16, 17, 18, 19, 20, 21],
                [0, 22, 23, 24, 25, 26, 27, 28],
                [0, 29, 30, 31, 32, 33, 34, 35],
                [0, 36, 37, 38, 39, 40, 41, 42],
                [0, 43, 44, 45, 46, 47, 48, 49],
                [0, 50, 51, 52, 53, 54, 55, 56],
                [1, 8, 15, 22, 29, 36, 43, 50],
                [1, 9, 16, 23, 30, 37, 44, 51],
                [1, 10, 17, 24, 31, 38, 45, 52],
                [1, 11, 18, 25, 32, 39, 46, 53],
                [1, 12, 19, 26, 33, 40, 47, 54],
                [1, 13, 20, 27, 34, 41, 48, 55],
                [1, 14, 21, 28, 35, 42, 49, 56],
                [2, 8, 16, 24, 32, 40, 48, 56],
                [2, 9, 17, 25, 33, 41, 49, 50],
                [2, 10, 18, 26, 34, 42, 43, 51],
                [2, 11, 19, 27, 35, 36, 44, 52],
                [2, 12, 20, 28, 29, 37, 45, 53],
                [2, 13, 21, 22, 30, 38, 46, 54],
                [2, 14, 15, 23, 31, 39, 47, 55],
                [3, 8, 17, 26, 35, 37, 46, 55],
                [3, 9, 18, 27, 29, 38, 47, 56],
                [3, 10, 19, 28, 30, 39, 48, 50],
                [3, 11, 20, 22, 31, 40, 49, 51],
                [3, 12, 21, 23, 32, 41, 43, 52],
                [3, 13, 15, 24, 33, 42, 44, 53],
                [3, 14, 16, 25, 34, 36, 45, 54],
                [4, 8, 18, 28, 31, 41, 44, 54],
                [4, 9, 19, 22, 32, 42, 45, 55],
                [4, 10, 20, 23, 33, 36, 46, 56],
                [4, 11, 21, 24, 34, 37, 47, 50],
                [4, 12, 15, 25, 35, 38, 48, 51],
                [4, 13, 16, 26, 29, 39, 49, 52],
                [4, 14, 17, 27, 30, 40, 43, 53],
                [5, 8, 19, 23, 34, 38, 49, 53],
                [5, 9, 20, 24, 35, 39, 43, 54],
                [5, 10, 21, 25, 29, 40, 44, 55],
                [5, 11, 15, 26, 30, 41, 45, 56],
                [5, 12, 16, 27, 31, 42, 46, 50],
                [5, 13, 17, 28, 32, 36, 47, 51],
                [5, 14, 18, 22, 33, 37, 48, 52],
                [6, 8, 20, 25, 30, 42, 47, 52],
                [6, 9, 21, 26, 31, 36, 48, 53],
                [6, 10, 15, 27, 32, 37, 49, 54],
                [6, 11, 16, 28, 33, 38, 43, 55],
                [6, 12, 17, 22, 34, 39, 44, 56],
                [6, 13, 18, 23, 35, 40, 42, 50],
                [6, 14, 19, 24, 29, 41, 46, 51],
                [7, 8, 21, 27, 33, 39, 45, 54],
                [7, 9, 15, 28, 34, 40, 46, 52],
                [7, 10, 16, 22, 35, 41, 47, 53],
                [7, 11, 17, 23, 29, 42, 48, 54],
                [7, 12, 18, 24, 30, 36, 49, 55],
                [7, 13, 19, 25, 31, 37, 43, 56],
                [7, 14, 20, 26, 32, 38, 44, 50]
            ]
        };

        /*****************
         * Data Computed *
         *****************/
        self.matrice = ko.computed(function() {
            return self.matrices["nb-cards-" + self.nbCards()];
        });

        self.availableLists = ko.computed(function() {
            return Object.keys(self.lists);
        });

        /********************
         * Service Computed *
         ********************/
        self.generateWords = ko.computed(function() {
            var o = self.lists[self.list()];
            for(var j, x, i = o.length; i; j = Math.floor(Math.random() * i), x = o[--i], o[i] = o[j], o[j] = x);
            self.words(o.splice(0, self.nbCards()));
        });
    };
});